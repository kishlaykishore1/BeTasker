...
protocol MICollectionViewBubbleLayoutDelegate: AnyObject {
...
open class MICollectionViewBubbleLayout: UICollectionViewFlowLayout {
    ...
    private func inset(for section: Int) -> UIEdgeInsets {
        guard let collectionView = collectionView,
              let delegate = delegate else {
            return sectionInset // fallback
        }
    ...
    fileprivate func alignmentAxis(for section: Int) -> AlignmentAxis<HorizontalAlignment>? {
        switch effectiveHorizontalAlignment {
        case .left:
            return AlignmentAxis(alignment: HorizontalAlignment.left, position: inset(for: section).left)
        case .right:
            guard let collectionViewWidth = collectionView?.frame.size.width else {
                return nil
            }
            return AlignmentAxis(alignment: HorizontalAlignment.right, position: collectionViewWidth - inset(for: section).right)
        default:
            return nil
        }
    }
    ...
    private var contentWidth: CGFloat? {
        ...
        return collectionViewWidth - inset(for: 0).left - inset(for: 0).right
    }
    ...
    fileprivate func isFrame(for firstItemAttributes: UICollectionViewLayoutAttributes, inSameLineAsFrameFor secondItemAttributes: UICollectionViewLayoutAttributes) -> Bool {
        ...
        let lineFrame = CGRect(x: inset(for: firstItemAttributes.indexPath.section).left,
    ...
    fileprivate func layoutAttributes(forItemsInLineWith layoutAttributes: UICollectionViewLayoutAttributes) -> [UICollectionViewLayoutAttributes] {
        ...
        lineFrame.origin.x = inset(for: section).left
...
fileprivate extension UICollectionViewLayoutAttributes {
    ...
    func alignHorizontally(collectionViewLayout: MICollectionViewBubbleLayout) {
        ...
        guard let alignmentAxis = collectionViewLayout.alignmentAxis(for: indexPath.section) else {
            return
        }
