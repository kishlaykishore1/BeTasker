...
class CircularSlider: UIControl {
    ...
    var currentValueWithoutAction: Float {
        set {
            assert(newValue <= maximumValue && newValue >= minimumValue, "current value \(newValue) must be between minimumValue \(minimumValue) and maximumValue \(maximumValue)")
            // Update the angleFromNorth to match this newly set value
            angleFromNorth = Int((newValue * Float(maximumAngle)) / (maximumValue - minimumValue))
    ...
    // Value to set fill gradient or not
    var isFillGradientApply: Bool = false {
        didSet {
            setNeedsDisplay()
        }
    }
    ...
    var filledGradientColors: [UIColor] = [.black, .lightGray] {
        didSet {
            setNeedsDisplay()
        }
    }
    ...
    fileprivate var computedHandleColor: UIColor? {
        ...
        case .bigWhiteCircle:
            newHandleColor = UIColor(white: 1.0, alpha: 1.0)
        case .noColor:
    ...
    func drawLine(_ ctx: CGContext) {
        ...
        if isFillGradientApply {
            CircularTrig.drawfilledGradientArcInContext(ctx, center: centerPoint, radius: computedRadius, lineWidth: CGFloat(lineWidth), fromAngleFromNorth: 0, toAngleFromNorth: CGFloat(angleFromNorth), colors: filledGradientColors, lineCap: filledArcLineCap)
        }
    ...
    func drawHandle(_ ctx: CGContext, atPoint handleCenter: CGPoint) -> CGRect {
        ...
        // colors
        let shadowColor = UIColor(white: 0.0, alpha: 0.2)
