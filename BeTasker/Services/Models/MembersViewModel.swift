...
struct MembersModel: Codable {
    var is_already: Bool?
    var member_data: [MembersDataModel]?
}

struct MembersDataModel: Codable {
    var id: Int? //8,
...
struct MembersDataViewModel {
    private var data = MembersDataModel()
    init(data: MembersDataModel) {
        self.data = data
    }
    var id: Int {
        get {
...
struct MembersViewModel {
    private var data = MembersModel()
    init(data: MembersModel) {
        self.data = data
    }
    
    static var blank: MembersViewModel {
        return MembersViewModel(data: MembersModel(is_already: nil, member_data: nil))
    }
    
    var arrMembers: [MembersDataViewModel] {
        if let arr = data.member_data {
            return arr.map({MembersDataViewModel(data: $0)})
        }
        return []
    }
    
    var isAlreadyAdded: Bool {
        return data.is_already ?? false
    }
    
    static func SearchMembersList(groupId: Int, searchKeyword: String, shouldShowInfo: Bool = false, sender: UIViewController, completion: @escaping(_ searchedMember: MembersViewModel)->()) {
        ...
        HpAPI.membersSearch.DataAPI(params: params, shouldShowError: false, shouldShowSuccess: false, shouldShowInfo: shouldShowInfo, key: "data") { (response: Result<MembersModel, Error>) in
            DispatchQueue.main.async {
                ...
                case .success(let res):
                    ...
                    break
                case .failure(_):
                    completion(MembersViewModel.blank)
                    break
                }
            }
        }
    }
    static func GetMembersList(groupId: Int, page: Int, limit: Int, sender: UIViewController, shouldShowLoader: Bool, completion: @escaping(_ arrMembers: [MembersDataViewModel])->()) {
        ...
        HpAPI.membersList.DataAPI(params: params, shouldShowError: false, shouldShowSuccess: false, key: "data") { (response: Result<MembersModel, Error>) in
            DispatchQueue.main.async {
                if shouldShowLoader {
                    Global.dismissLoadingSpinner(sender.view)
                }
    ...
    static func GetWorkSpaceMembersList(workSpaceId: Int, page: Int, limit: Int, sender: UIViewController, shouldShowLoader: Bool, completion: @escaping(_ arrMembers: [MembersDataViewModel])->()) {
        var params: [String: Any] = [
            "lc": Constants.lc,
            "page": page,
            "limit": limit,
        ...
        HpAPI.workSpacemembersList.DataAPI(params: params, shouldShowError: false, shouldShowSuccess: false, key: nil) { (response: Result<MembersModel, Error>) in
            DispatchQueue.main.async {
                Global.dismissLoadingSpinner(sender.view)
                switch response {
                case .success(let res):
